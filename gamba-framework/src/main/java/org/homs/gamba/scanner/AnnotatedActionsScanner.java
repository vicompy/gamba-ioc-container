...
public class AnnotatedActionsScanner {
	...
	private final static Log log = new Log(AnnotatedActionsScanner.class);
	...
	public Map<String, DeclaredAction> doScan(String actionBasePackage) {
		...
		if (actionBasePackage == null) {
			...
			log.warning("front-controller init-param not specified, using the default value: ",
					ACTIONS_BASE_PACKAGE_DEFAULT_VALUE);
//			System.out
//					.println("warning: front-controller init-param not specified, using the default value: "
//							+ actionBasePackage);
		}
			...
			for (final Method m : c.getMethods()) {
				...
				if (actionAnnotation != null) {
					...
					final DeclaredAction da = new DeclaredAction(actionAnnotation.name(), c, m,
							actionAnnotation.formBean(), actionAnnotation.validator(), actionAnnotation
									.onValidationError());
		...
		log.info("explorat package d'actions: ", actionBasePackage);
		log.info("actions trobades: ", declaredActions.size());
