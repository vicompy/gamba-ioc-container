...
public class BeanBinder implements IBinder {

	protected Object bind(final BeanInfo cachedBean, final Map<String, Object> atr) {
		...
		for (final String atrName : atr.keySet()) {
			...
			if (atrName.equals(beanProp.propertyName)) {
				...
				try {
					beanProp.method.invoke(bean, convert(atr.get(atrName), beanProp.argType));
				} catch (final Exception exc) {
					// TODO Auto-generated catch block
					exc.printStackTrace();
				}
	...
	private Object convert(final Object value, final Class<?> type) {
		if (type.isArray()) {
			...
			if (type.isPrimitive()) {
				return convertToPrimitive(type, v);
			} else {
				return convertToPrimitiveWrapper(type, v);
			}
		}
	}

	private Object convertToPrimitive(final Class<?> type, final String v) {
		if (int.class.equals(type)) return Integer.valueOf(v);
		if (long.class.equals(type)) return Long.valueOf(v);
		if (float.class.equals(type)) return Float.valueOf(v);
		if (double.class.equals(type)) return Double.valueOf(v);
		if (boolean.class.equals(type)) return Boolean.valueOf(v); // TODO si?
		if (short.class.equals(type)) return Short.valueOf(v);
		throw new NullPointerException();
	}
	private Object convertToPrimitiveWrapper(final Class<?> type, final String v) {
		if (Integer.class.equals(type)) return Integer.valueOf(v);
		if (Long.class.equals(type)) return Long.valueOf(v);
		if (Float.class.equals(type)) return Float.valueOf(v);
		if (Double.class.equals(type)) return Double.valueOf(v);
		if (Boolean.class.equals(type)) return Boolean.valueOf(v); // TODO si?
	...
	private Object[] newArray(final Class<?> type, final int size) {
		if (type.isPrimitive()) {
			...
			if (Integer.class.equals(type)) return new Integer[size];
			if (Long.class.equals(type)) return new Long[size];
			if (Float.class.equals(type)) return new Float[size];
			if (Double.class.equals(type)) return new Double[size];
			if (Boolean.class.equals(type)) return new Boolean[size]; // TODO si?
			if (Short.class.equals(type)) return new Short[size];
			if (String.class.equals(type)) return new String[size];
		}
		return null;
	}
	...
	protected BeanInfo analitza(final Class<?> beanClass) {
		...
		for (final Method m : beanClass.getMethods()) {
			if (isSetter(m.getName()) && m.getParameterTypes().length == 1) {
				final BeanPropInfo bp = new BeanPropInfo(deSet(m.getName()), m.getName(), m, m.getParameterTypes()[0]);
				cb.addBeanProp(bp);
			}
		}
		return cb;
