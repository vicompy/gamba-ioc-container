...
public class ConstrTag {
	...
	public ConstrTag(final String ref, final String value, final String type) throws GambaException {
		super();
		this.ref = ref;
		this.value = value;
		this.type = type;
		...
		try {
			if (ref == null) {
				...
				if (value != null) {
					throw new GambaException("'value' atribute must be null");
				}
				if (type != null) {
					throw new GambaException("'type' atribute must be null");
				}
			}
		} catch (final GambaException e) {
			throw new GambaException("bean definition attributes: ref | (value [type])", e);
		}
	}
