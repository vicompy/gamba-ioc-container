...
public final class DebugLevel {
	...
	/**
	 * A map of all {@link DebugLevel}s, keyed by the lower-case version of
	 * their name (without the Lx_ prefix).
	 */
	private static final Map levelsByName;

	static {
		final HashMap levels = new HashMap(10, 0.8F);
	 ...
	 */
	private DebugLevel(final int level, final String name) {
		this.level = level;
		this.name = name;
	}
	 ...
	 */
	boolean shouldLog(final DebugLevel logLevel) {
	 ...
	 *
	 * @param level the level value whose corresponding <code>DebugLevel</code>
	 *        is to be returned.
	 * @return The matching or closest matching <code>DebugLevel</code>.
	 */
	public static DebugLevel fromInt(final int level) {
		switch (level) {
			case 1:
				return L1_FATAL;
			case 2:
	 ...
	 * case-insensitively.
	 *
	 * @param name the name of the <code>DebugLevel</code> to return. This name
	 ...
	 *         or if there is no <code>DebugLevel</code> associated with the
	 *         specified name.
	 */
	static DebugLevel fromName(final String name) throws IllegalArgumentException {
		if (name == null)
			throw new IllegalArgumentException("name cannot be null.");

		final DebugLevel level = (DebugLevel) levelsByName.get(name.toLowerCase());
		if (level == null)
