...
/**
 * <p>
 * Controls the configuration and formatting of a group of
 * <code>SimpleLogger</code>s.
 * </p>
 * <p>
 * <code>SimpleLog</code> has a default instance, which is probably the only one
 * that anyone will ever use. If you use the default instance, you don't even
 * really need to know anything about <code>SimpleLog</code> - just use the
 * {@link SimpleLogger#SimpleLogger(Class) basic SimpleLogger constructor} and
 * you'll never even know nor care.
 * </p>
 *
...
public final class SimpleLog {
	...
	/**
	 * The default name of the properties file used to configure a
	 * <code>SimpleLog</code>.
	 */
	...
	/**
	 * The property key for whether output to a log file should also be printed
	 * to the console.
	 */
	...
	/**
	 * The period (in milliseconds) between checks of the properties file (must
	 * be a FILE, not inside a JAR).
	 */
	private static final int RELOAD_FILE_CHECK_PERIOD = 20 * 1000;

	/**
	 * The period (in milliseconds) between checks of the properties when
	 * they're not in a file.
	 */
	...
	/**
	 * The string to use to print a new line between the debug exception line
	 * and the stack trace.
	 */
	...
	/**
	 * An object to synchronize on when checking or changing the
	...
	static {
		try {
			final String devDebugString = System.getProperty(DEV_DEBUG_PROPERTY);
	...
	/**
	 * The URL from where this <code>SimpleLog</code> loaded (and may reload)
	 * its configuration.
	 */
	...
	/**
	 * The path and name of the log file being written to, or <code>null</code>
	 * if output is going somewhere else.
	 */
	...
	/**
	 * The non-instance {@link SimpleLogger}s attached to this
	 * <code>SimpleLog</code>.
	 */
	private final List loggers = new ArrayList(32);

	/**
	 * A list of {@link WeakReference}s to instance {@link SimpleLogger}s
	 * attached to this <code>SimpleLog</code>.
	 */
	...
	/**
	 * An object to synchronize on when accessing or modifying the
	 * {@link #loggers} list.
	 */
	...
	/**
	 * Message format for a simple debug message. 4 = message (String)
	 */
	private MessageFormat dbFormat;

	/**
	 * Message format for a debug object message. 4 = object name (String) 5 =
	 * object value (Object)
	 */
	private MessageFormat dboFormat;

	/**
	 * Message format for a debug exception message. 4 = exception (Throwable)
	 */
	private MessageFormat dbeFormat;

	/**
	 * Message format for a trace entry message. 4 = method name (String)
	 */
	private MessageFormat entryFormat;

	/**
	 * Message format for a trace exit message. 4 = method name (String)
	 */
	private MessageFormat exitFormat;

	/**
	 * Message format for a simple debug message (instance version). 4 = message
	 * (String)
	 */
	private MessageFormat dbFormat4Instance;

	/**
	 * Message format for a debug object message (instance version). 4 = object
	 * name (String) 5 = object value (Object)
	 */
	private MessageFormat dboFormat4Instance;

	/**
	 * Message format for a debug exception message (instance version). 4 =
	 * exception (Throwable)
	 */
	private MessageFormat dbeFormat4Instance;

	/**
	 * Message format for a trace entry message (instance version). 4 = method
	 * name (String)
	 */
	private MessageFormat entryFormat4Instance;

	/**
	 * Message format for a trace exit message (instance version). 4 = method
	 * name (String)
	 */
	private MessageFormat exitFormat4Instance;

	/**
	 * Creates a new <code>SimpleLog</code> configured by the given properties
	 * object. <br>
	 * All <code>SimpleLog</code>s log to System.err by default.
	 *
	 * @param properties a properties object containing properties that will be
	 *        used to configure the <code>SimpleLog</code>.
	 * @throws IllegalArgumentException if <code>properties</code> is
	 *         <code>null</code>.
	 */
	public SimpleLog(final Properties properties) {
		if (properties == null) {
	...
	/**
	 * Creates a new <code>SimpleLog</code> configured by a properties file read
	 * from the given URL.
	 *
	 * @param configurationSource the properties file to use to configure the
	 *        <code>SimpleLog</code> instance.
	 * @throws IOException if the properties file cannot be read from the given
	 *         URL.
	 */
	public SimpleLog(final URL configurationSource) throws IOException {
		...
		final String reloadingString = properties.getProperty(KEY_RELOADING, RELOADING_DEFAULT);
		final boolean reloading = Boolean.valueOf(reloadingString).booleanValue();
		if (reloading) {
	...
	/**
	 * <p>
	 * Creates a new <code>SimpleLog</code> configured by a specified properties
	 * file.
	 * </p>
	 * <p>
	 * Note that this is merely a convenience method equivalent to:
	 * <code>new SimpleLog(configurationSource.toURL())</code>
	 * </p>
	 *
	 * @param configurationSource the properties file to use to configure the
	 *        <code>SimpleLog</code> instance.
	 * @throws IOException if an error occurs interpreting the file name as a
	 *         URL or if the properties file cannot be read.
	 */
	public SimpleLog(final File configurationSource) throws IOException {
		this(configurationSource.toURL());
	}

	private void setConsole(final Properties properties) {
		final String console = properties.getProperty(KEY_CONSOLE);
	...
	/**
	 * Loads the properties file from this <code>SimpleLog</code>'s
	 * configuration source and reads all the special properties used to
	 * configure the <code>SimpleLog</code> (as opposed to those that configure
	 * the attached <code>SimpleLogger</code>s).
	 *
	...
	private void loadProperties() throws IOException {
		...
		// Load the properties into a new object, then replace the current ones
		// if the read suceeds.
		final InputStream inputStream = configurationSource.openStream();
		...
		if (importList != null) {
			...
			if (importList.length() > 0) {
				final String[] filesToImport = importList.split(",");
				if (filesToImport != null && filesToImport.length != 0) {
					...
					for (int i = 0; i < filesToImport.length; i++) {
						final String filenameToImport = filesToImport[i];
						final URL urlToImport = new URL(configurationContext + filenameToImport);
						...
						try {
							...
							printError(
									"Error importing properties file: " + filenameToImport + "(" + urlToImport + ")",
									e, true);
						} finally {
		...
		if (devDebug) {
			final Set properties = newProperties.entrySet();
	...
	/**
	 * Reads settings from this <code>SimpleLog</code>'s properties object and
	 * changes the object's state accordingly.
	 */
	private void readSettingsFromProperties() {
		if (!outputSetProgramatically) {
			...
			try {
				final String rolloverStrategyString = properties.getProperty(KEY_ROLLOVER_STRATEGY);
				final boolean useRollover = false;
//				TODO comentat
//				= rolloverStrategyString != null
//						&& rolloverStrategyString.trim().length() != 0;
...
//				}
		...
		final String pipeOutputToConsoleString = properties.getProperty(KEY_PIPE_TO_CONSOLE);
		// The strategy here is to only turn andConsole on if the property
		// definitely says true
		...
		if (defaultLevelStr != null) {
			...
			try {
				...
				final int level = Integer.parseInt(defaultLevelStr);
		...
		final String defaultTraceStr = properties.getProperty(KEY_DEFAULT_TRACE);
		if (defaultTraceStr != null) {
			defaultTracing = Boolean.valueOf(defaultTraceStr).booleanValue();
		}

		// Read the Date format
		final String dateFormatString = properties.getProperty(KEY_DATE_FORMAT, DATE_FORMAT_DEFAULT);
		try {
		...
		dbFormat4Instance = readFormat(KEY_FORMAT_DB + KEY_FORMAT_INSTANCE_SUFFIX, DEFAULT_FORMAT_STRING_DB_INSTANCE);
		dboFormat4Instance = readFormat(KEY_FORMAT_DBO + KEY_FORMAT_INSTANCE_SUFFIX, DEFAULT_FORMAT_STRING_DBO_INSTANCE);
		dbeFormat4Instance = readFormat(KEY_FORMAT_DBE + KEY_FORMAT_INSTANCE_SUFFIX, DEFAULT_FORMAT_STRING_DBE_INSTANCE);
		entryFormat4Instance = readFormat(KEY_FORMAT_ENTRY + KEY_FORMAT_INSTANCE_SUFFIX,
				DEFAULT_FORMAT_STRING_ENTRY_INSTANCE);
		exitFormat4Instance = readFormat(KEY_FORMAT_EXIT + KEY_FORMAT_INSTANCE_SUFFIX,
				DEFAULT_FORMAT_STRING_EXIT_INSTANCE);

		updateDateFormats();

		// Read the Print stack traces property
		final String printStackTracesStr = properties.getProperty(KEY_PRINT_STACK_TRACES, PRINT_STACK_TRACES_DEFAULT);
		final boolean printStackTraces = Boolean.valueOf(printStackTracesStr).booleanValue();
		if (printStackTraces) {
			final ExceptionFormat exceptionFormat = new ExceptionFormat();
			dbeFormat.setFormatByArgumentIndex(DEFAULT_FORMAT_EXCEPTION_INDEX, exceptionFormat);
			dbeFormat4Instance.setFormatByArgumentIndex(DEFAULT_FORMAT_EXCEPTION_INDEX_INSTANCE, exceptionFormat);
		}
	...
	/**
	 * Configures this <code>SimpleLog</code> to use a plain FileWriter,
	 * according to the properties in the current properties object.
	 *
	...
	private Writer configureFileWriter() throws IOException {
		...
		final String interpretNameStr = properties.getProperty(KEY_INTERPRET_NAME);
		// The strategy here is to only turn interpret off if the property
		// definitely says false
		...
		final boolean newLogFileNotNull = newLogFile != null;
		final boolean nameContainsBraces = newLogFileNotNull && newLogFile.indexOf('{') != -1;
		if (newLogFileNotNull && nameContainsBraces && interpretName) {
			try {
				final MessageFormat logFileNameFormat = new MessageFormat(newLogFile);
		...
		// The log file has changed if it used to be null and now isn't, or vice
		// versa...
		boolean logFileChanged = (logFile == null) != (newLogFile == null);

		// or it's changed if it wasn't null and still isn't null but the name
		// has changed.
		logFileChanged |= logFile != null && newLogFileNotNull && !newLogFile.equals(logFile);

		// or it's changed if rollover was on before (but it's now off if we're
		// in here)
		final boolean rolloverWasInUse = currentWriter != null
		...
		if (logFileChanged) {
			if (newLogFile == null) {
				...
				final File file = new File(newLogFile).getAbsoluteFile();
				...
				final String appendStr = properties.getProperty(KEY_APPEND);
				// The strategy here is to only turn append off if the property
				// definitely says
				// false
				if (appendStr != null) {
	...
	/**
	 * Configures this <code>SimpleLog</code> to use a {@link RolloverManager},
	 * according to the properties in the current properties object.
	 *
	 * @return the writer to be used to write log output.
	 */
...
//		// Test we've got the RolloverManager class, so we can throw something
//		// really meaningful.
//		try {
...
//			// Ignore anything else that might be thrown (e.g.
//			// SecurityException).
//		}
//
//		Writer writer;
...
//	}
	 ...
	 *
	 * @param key the key to read from the properties to obtain a new message
	 *        format string.
	 * @param defaultPattern the default pattern to use if the property doesn't
	 *        exist.
	 * @return a new MessageFormat, created from the resulting message format
	 *         string.
	 */
	private MessageFormat readFormat(final String key, final String defaultPattern) {
		final String formatString = properties.getProperty(key, defaultPattern);
	...
	/**
	 * <p>
	 * Sets the default instance of <code>SimpleLog</code>.
	 * </p>
	 * <p>
	 * If the default instance has already been initialised and has been used to
	 * create one or more {@link SimpleLogger}s, this method will print a
	 * warning on <code>System.err</code> but the operation will still be
	 * completed.
	 * </p>
	 *
	 * @param log the SimpleLog instance to be used as the default instance
	 */
	public static void setDefaultInstance(final SimpleLog log) {
		if (log == null)
		...
		synchronized (defaultInstanceLock) {
			...
			if (defaultInstance != null
					&& (!defaultInstance.loggers.isEmpty() || !defaultInstance.instanceLoggerReferences.isEmpty())) {
	...
	/**
	 * <p>
	 * Returns the default instance of <code>SimpleLog</code>. If one has not
	 * been set manually, a default instance is created and initialised
	 * automatically.
	 * </p>
	 * <p>
	 * The automatically-initialised default instance is configured:
	 * <ul>
	 * <li>from a properties file specified in the value of the system property
	 * <code>simplelog.configuration</code> via the classpath or file system,
	 * depending on whether the property value begins with
	 * <code>classpath:</code> or <code>file:</code></li>
	 * <li>or, if the system property is not specified, from a properties file
	 * found on the classpath with the name 'simplelog.properties' <i>(this is
	 * the most usual method of configuration)</i></li>
	 * <li>or, if that file is not found in the classpath, not configured at
	 * all, in which case Simple Log will produce no output whatsoever unless a
	 * writer is programmatically assigned (see {@link #setWriter}).</li>
	 * </ul>
	 * </p>
	 * <p>
	 * If your chosen method of configuration does not appear to be working, you
	 * can set the value of the <code>simplelog.dev.debug</code> system property
	 * to <code>true</code> to see where Simple Log is looking for its
	 * configuration.
	 * </p>
	 *
	...
	public static SimpleLog defaultInstance() {
		synchronized (defaultInstanceLock) {
			if (defaultInstance == null) {
				...
				final URL propertiesUrl = getPropertiesUrl();
	...
	private static URL getPropertiesUrl() {
		...
		if (propertiesDefinition != null) {
			...
			if (propertiesDefinition.startsWith(FILE_CONFIG_PREFIX)) {
				final String propertiesLocation = propertiesDefinition.substring(FILE_CONFIG_PREFIX.length());
			...
			// Classpath
				...
				final String propertiesLocation = propertiesDefinition.substring(CLASSPATH_CONFIG_PREFIX.length());
			...
			// Junk
				...
				printError(CONFIG_PROPERTY + " property must begin with '" + FILE_CONFIG_PREFIX + "' or '"
						+ CLASSPATH_CONFIG_PREFIX + "' ('" + propertiesDefinition + "')");
			}
		}
	...
	/**
	 * Prints the given string to this <code>SimpleLog</code>'s output
	 * destination, followed by a newline sequence.
	 *
	...
	/**
	 * Configures the given logger by setting its debug level and trace flag
	 * according to the current properties.
	 *
	 * @param logger the logger to configure.
	 */
	void configure(final SimpleLogger logger) {
	 ...
	 */
	private DebugLevel getDebugLevel(final SimpleLogger logger) {
		if (properties == null) {
			return defaultLevel;
		}
		...
		do {
			// On first iteration, this substring() call returns the whole
			// string.
			// On subsequent iterations, it removes everything after and
	 ...
	 */
	private boolean getTracingFlag(final SimpleLogger logger) {
		if (properties == null) {
		...
		do {
			loggerConfigName = loggerConfigName.substring(0, dotIndex);
			final String value = properties.getProperty(loggerConfigName + TRACE_SUFFIX);
			if (value != null) {
	...
	/**
	 * Registers the give logger with this <code>SimpleLog</code>, so that the
	 * logger can be updaed if the properties change.
	 *
	 * @param logger the logger to register
	 */
	void register(final SimpleLogger logger) {
		synchronized (LOGGERS_LOCK) {
			if (!logger.isInstanceDebugger()) {
				loggers.add(logger);
			} else {
				// Instance loggers get weak referenced, because they're much
				// more likely to be GC'd.
	...
	/**
	 * Creates the {@link ReferenceQueue} for instance loggers and starts a
	 * daemon thread to clear queue.
	 */
		...
		final Thread thread = new Thread(new Runnable() {
			public void run() {
				while (true) {
					try {
						Thread.yield();
						final Reference reference = instanceLoggersReferenceQueue.remove();
						synchronized (LOGGERS_LOCK) {
	...
	/**
	 * Re-configures all {@link SimpleLogger}s registered with this
	 * <code>SimpleLog</code>.
	 */
	private void reconfigureAllLoggers() {
		synchronized (LOGGERS_LOCK) {
			...
			for (final Iterator iter = loggers.iterator(); iter.hasNext();) {
				configure((SimpleLogger) iter.next());
			}

			for (final Iterator iter = instanceLoggerReferences.iterator(); iter.hasNext();) {
				final Reference loggerReference = (Reference) iter.next();
				final SimpleLogger logger = (SimpleLogger) loggerReference.get();
				if (logger != null)
					configure(logger);
			}
		}
	...
	/**
	 * Prints the given debug message to System.err if Simple Log dev debugging
	 * is enabled.
	 *
	 * @param message a message to print
	 */
	private static void printDebugIfEnabled(final String message) {
		if (devDebug)
			System.err.println("SimpleLog [dev.debug]: " + message);
	}

	/**
	 * Prints the given debug message and object value to System.err if Simple
	 * Log dev debugging is enabled.
	 *
	 * @param message a message to print
	 * @param value an object to print
	 */
	private static void printDebugIfEnabled(final String message, final Object value) {
		if (devDebug)
	 ...
	 */
	private static void printError(final String description) {
	 ...
	 *
	 * @param description a description the error
	 * @param error the exception that occurred to cause the error (may be
	 *        <code>null</code>)
	 * @param printExceptionType whether the whole toString of the exception
	 *        should be printed (true) of just the exception's 'message'
	 *        (false).
	 */
	private static void printError(final String description, final Throwable error, final boolean printExceptionType) {
		boolean printStackTraces = false;
		try {
			final String printStackTracesStr = System.getProperty(DEV_STACKTRACES_PROPERTY);
			printStackTraces = printStackTracesStr != null && printStackTracesStr.trim().equalsIgnoreCase("true");
		} catch (final SecurityException e) {
			...
			if (error != null) {
				...
				if (printStackTraces) {
					try {
						...
						// Ignore SecurityExceptions from trying to print stack
						// traces
					}
				}
	 ...
	 */
	public void setDefaultLevel(final DebugLevel defaultLevel) {
	 ...
	 *
	 * @param defaultTracing the new default trace value (<code>true</code> for
	 *        on, <code>false</code> for off).
	 */
	public void setDefaultTracing(final boolean defaultTracing) {
	...
	/**
	 * Sets this <code>SimpleLog</code>'s default date format. If the given
	 * value is <code>null</code>, the default date format will be used.
	 *
	...
	/**
	 * Returns the print writer that is the destination for all of this
	 * <code>SimpleLog</code>'s output.
	 *
	 * @return the print writer being used by this <code>SimpleLog</code>, or
	...
	/**
	 * Sets the print writer to be used as the destination for all of this
	 * <code>SimpleLog</code>'s output.
	 *
	 * @param out the print to be used by this <code>SimpleLog</code>, or
	 *        <code>null</code> if it should not create any output.
	 */
	...
	/**
	 * Returns whether this <code>SimpleLog</code> is currently printing output
	 * at all.
	 */
	...
	/**
	 * Updates the date format in all of this <code>SimpleLog</code>'s message
	 * formats.
	 */
	...
	/**
	 * Returns whether this <code>SimpleLog</code>, when printing output to a
	 * file, will also print the output to the console. This attribute has no
	 * effect when output is not going to a file.
	 *
	 * @return <code>true</code> if output will be piped to the console,
	...
	/**
	 * Sets whether this <code>SimpleLog</code>, when printing output to a file,
	 * should also print the output to the console. This attribute has no effect
	 * when output is not going to a file.
	 *
	 * @param pipeOutputToConsole <code>true</code> if output should be piped to
	 *        the console as well as the output file, <code>false</code> if it
	 *        should not.
	 */
	public void setPipingOutputToConsole(final boolean pipeOutputToConsole) {
		this.pipingOutputToConsole = pipeOutputToConsole;
	}

	/**
	 * Format implementation that prints the stack trace of an exception after
	 * the exception object's toString.
	 */
	private static final class ExceptionFormat extends Format {
		@Override
	...
	/**
	 * A {@link TimerTask} that checks to see if the last modified date of the
	 * configuration source (which must be a "file" URL) has changed and, if it
	 * has, reloads the <code>SimpleLog</code>'s configuration.
	 */
	private final class FileConfigurationReloader extends TimerTask {
		...
		public FileConfigurationReloader() {
			try {
				final URI uri = new URI(configurationSource.toExternalForm());
	...
	/**
	 * A {@link TimerTask} that checks to see if the last modified date of the
	 * configuration source has changed and, if it has, reloads the
	 * <code>SimpleLog</code>'s configuration. For configuration sources that
	 * cannot provide a last modified date, the properties in the configuration
	 * source are loaded on every iteration and compared to the current
	 * properties.
	 */
	private final class UrlConfigurationReloader extends TimerTask {
		...
		public UrlConfigurationReloader() {
			try {
				final URLConnection connection = configurationSource.openConnection();
...
//	TODO comentat
//	private static class ErrorReporter implements RolloverManager.ErrorReporter {
//		public void error(final String description, final Throwable t, final boolean printExceptionType) {
//			printError(description, t, printExceptionType);
//		}
//
//		private static RolloverManager.ErrorReporter create() {
